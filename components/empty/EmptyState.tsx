...
export function EmptyState({
  ...
  return (
    <div className="flex flex-col items-center justify-center px-5 py-16 text-center">
      <div className="mb-6 flex h-16 w-16 -rotate-6 items-center justify-center rounded-[23px] bg-muted text-primary shadow-sm">
        <Icon size={28} weight="duotone" />
      </div>
      <h3 className="text-xl font-medium tracking-tight">{t(headline)}</h3>
      {subcopy ? (
        <p className="mt-3 max-w-sm text-sm leading-7 text-muted-foreground">{t(subcopy)}</p>
      ) : null}
      {(primary || secondary) && (
